...
void EventLoop::handleRead()
{
    ...
    if(n != sizeof one){
        LOG_ERROR("EventLoop::handleRead() reads %lld bytes instead of 8", static_cast<long long>(n));
    }
}
// 开启事件循环
void EventLoop::loop()
...
void EventLoop::wakeup()
{
    ...
    if(n != sizeof one){
        LOG_ERROR("EventLoop::wakeup() writes %lld bytes instead of 8", static_cast<long long>(n));
    }
